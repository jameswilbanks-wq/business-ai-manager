...
export function ConversationListItemCard({
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-2 border-b border-border pl-4 pr-2 py-3 transition-colors hover:bg-accent/50",
        active && "bg-accent"
      )}
    >
      <Link href={`/communication/${conversation.id}`} className="flex min-w-0 flex-1 items-start gap-3">
        ...
        <Button
          ...
          onClick={() =>
            startTransition(async () => {
              await updateConversationStatusAction(conversation.id, "open");
            })
          }
        >
          <RotateCcw /> Restaurar
